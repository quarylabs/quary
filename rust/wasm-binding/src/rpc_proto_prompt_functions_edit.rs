...
pub async fn return_edit_model_prompt(
    ...
    let model_yaml = parse_all_models
        .iter()
        .find_map(|m| m.1.models.iter().find(|m| m.name == model_name));
...
You are a data analyst at a company that uses a {{ database_type }} database. You've been asked to edit the following 
...
Please edit the transformation as specified by the user.
