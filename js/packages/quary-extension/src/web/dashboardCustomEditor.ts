  ...
  private async onMessage(
    ...
    switch (e.type) {
      case USE_GLOBAL_STATE_MESSAGE_TYPE_NOT_SET: {
        ...
        const items: Array<DashboardEditorDataItem> =
          dashboardData.value.items.map((item) => ({
            item,
            result: {
              type: 'loading',
            },
        ...
        for (const item of items) {
          const result = await services.database.runStatement(item.item.sql)
          if (isErr(result)) {
            item.result = {
              type: 'error' as const,
              error: result.error,
