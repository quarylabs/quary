...
//   protoc               unknown
// source: quary/service/v1/query_result.proto
...
export const QueryResult: MessageFns<QueryResult> = {
  encode(message: QueryResult, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    for (const v of message.columns) {
      QueryResultColumn.encode(v!, writer.uint32(10).fork()).join();
    }
    return writer;
  },
...
export const QueryResultColumn: MessageFns<QueryResultColumn> = {
  encode(message: QueryResultColumn, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.name !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): QueryResultColumn {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
