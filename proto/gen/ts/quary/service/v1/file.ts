...
export const FileSystem: MessageFns<FileSystem> = {
  encode(message: FileSystem, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    Object.entries(message.files).forEach(([key, value]) => {
      FileSystem_FilesEntry.encode({ key: key as any, value }, writer.uint32(10).fork()).join();
    });
    return writer;
  },
...
export const FileSystem_FilesEntry: MessageFns<FileSystem_FilesEntry> = {
  encode(message: FileSystem_FilesEntry, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.key !== "") {
      writer.uint32(10).string(message.key);
    }
    if (message.value !== undefined) {
      File.encode(message.value, writer.uint32(18).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): FileSystem_FilesEntry {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
