...
export const ChartFile: MessageFns<ChartFile> = {
  encode(message: ChartFile, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    switch (message.source?.$case) {
      ...
      case "preTemplatedSql":
        ...
        break;
      case "reference":
        ChartFile_AssetReference.encode(message.source.reference, writer.uint32(50).fork()).join();
        break;
    }
    if (message.config !== undefined) {
      Struct.encode(Struct.wrap(message.config), writer.uint32(58).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): ChartFile {
  ...
  toJSON(message: ChartFile): unknown {
    ...
    } else if (message.source?.$case === "preTemplatedSql") {
      obj.preTemplatedSql = message.source.preTemplatedSql;
    } else if (message.source?.$case === "reference") {
  ...
  fromPartial<I extends Exact<DeepPartial<ChartFile>, I>>(object: I): ChartFile {
    ...
    switch (object.source?.$case) {
      case "rawSql": {
        if (object.source?.rawSql !== undefined && object.source?.rawSql !== null) {
          message.source = { $case: "rawSql", rawSql: object.source.rawSql };
        }
        break;
      }
      case "preTemplatedSql": {
        if (object.source?.preTemplatedSql !== undefined && object.source?.preTemplatedSql !== null) {
          message.source = { $case: "preTemplatedSql", preTemplatedSql: object.source.preTemplatedSql };
        }
        break;
      }
      case "reference": {
        if (object.source?.reference !== undefined && object.source?.reference !== null) {
          message.source = {
            $case: "reference",
            reference: ChartFile_AssetReference.fromPartial(object.source.reference),
          };
        }
        break;
      }
...
export const ChartFile_AssetReference: MessageFns<ChartFile_AssetReference> = {
  encode(message: ChartFile_AssetReference, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.name !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): ChartFile_AssetReference {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
