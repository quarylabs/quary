...
export const Chart: MessageFns<Chart> = {
  encode(message: Chart, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.filePath !== "") {
      writer.uint32(74).string(message.filePath);
    }
    switch (message.source?.$case) {
      ...
      case "preTemplatedSql":
        ...
        break;
      case "reference":
        Chart_AssetReference.encode(message.source.reference, writer.uint32(50).fork()).join();
        break;
    }
...
export const Chart_AssetReference: MessageFns<Chart_AssetReference> = {
  encode(message: Chart_AssetReference, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.name !== "") {
      writer.uint32(10).string(message.name);
    }
    return writer;
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): Chart_AssetReference {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
